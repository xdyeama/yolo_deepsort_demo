...
class YOLOv8Detector:
	def __init__(self, detector_config: Dict[str, Any]):
		...
		global_config: merged config (optional) for device/precision preferences
		"""
		self.cfg = detector_config or {}

		# Resolve and configure device/runtime
		self.device = configure_torch_environment(self.global_cfg)
		...
		# Precision controls
		self.use_half = should_use_half(self.cfg, self.global_cfg, self.device)
		if self.use_half:
			try:
		...
		self.use_amp = should_use_amp(self.cfg, self.global_cfg, self.device)
	...
	def _infer(self, frames_rgb: List[np.ndarray]):
		"""
